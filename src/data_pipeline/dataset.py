    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        try:
            with np.load(npz_file_path) as npz_file:
                ...
                # --- CORRECTED: Load the keys that were actually saved ---
                seizure_labels = torch.from_numpy(npz_file['seizure_binary']).long()
                artifact_labels = torch.from_numpy(npz_file['artifact']).long()

                labels = {
                    'seizure_binary': seizure_labels,
                    'artifact': artifact_labels,
                }
